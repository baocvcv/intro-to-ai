						   ...
						   const int lastX, const int lastY, const int noX, const int noY)
{
	...
	// problem: which locations do we search more, which locations do we search less?
	// problem: identify high value locations; define "value" of a point
	// search, starting from places near lastX, lastY
	...
	long long time_d = duration_cast<milliseconds>(time1 - time0).count();
	// int time_d = 1;
	while (time_d < 1000) {
		// select new node
		...
		// simulate & backup
		// for (int i = 0; i < 10; i++) {
			int result = nodes[child_idx].defaultPolicy();
			backup(nodes, result, child_idx);
		// }

		time1 = high_resolution_clock::now();
		time_d = duration_cast<milliseconds>(time1 - time0).count();
		// time_d++;
	}
